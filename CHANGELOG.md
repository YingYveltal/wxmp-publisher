...
- **`link_url` 留空 = 渲染纯图无链接**（公众号合规，可保存）
  - 之前强制要求 grid/header 必须有 link_url，没拿到真实 URL 时 agent 会抓瞎
  - 现在留空即跳过校验，fill 输出无 `<a>` 包装的纯 `<img>`
  - 公众号只校验"有链接的图"，纯图不校验，可以正常保存
- 仍然拒绝填了"测试 URL"凑数的情况（含 placeholder 标记 / 公众号主页 URL / token 看起来是测试值）
- SKILL.md / INSTALL.md FAQ 加入"拿不到 URL 怎么办"
