...
## Workflow（5 个阶段，从工作区开始）

```
[创建工作区] → [写稿到 source.txt] → [scaffold] → [agent 加 inline+标关键词+对话分工]
                                                       ↓
                              [各方准图存 images/ + 回填 images.json] → [fill] → [md2wechat 渲染]
```

### 阶段 0：创建工作区（必做，不能跳过）

每篇文章都有自己的独立工作区目录，所有命令都在工作区内运行。

```bash
python3 ~/.hermes/skills/text2md/scripts/plan.py new "<theme>-<topic>"
# 例：new "edwards-蚁人超越加内特"
# 输出: 工作区已创建 ~/wxmp-articles/2026-04-20-edwards-蚁人超越加内特/
```

工作区根目录默认 `~/wxmp-articles/`（首次运行自动创建）。可通过环境变量 `WXMP_ARTICLES_DIR` 覆盖。

工作区结构：
```
~/wxmp-articles/2026-04-20-edwards-蚁人超越加内特/
├── .layout         # 工作区合法性标识（agent 不需关心）
├── source.txt      # 用户/agent 写入文字稿
├── draft.md        # scaffold 生成
├── images.json     # 配图清单
├── images/         # 所有图素材
├── final.md        # fill 生成
└── final.html      # md2wechat render 生成
```

**为什么强制工作区**：避免 agent 把中间文件散落各处。scaffold 和 fill 都会检查 `.layout` 标识，不在合法工作区直接报错（防误操作）。

### 阶段 1：把稿子写入 source.txt

agent 用 Write 工具把用户给的文字稿覆盖写入 `<workspace>/source.txt`（覆盖默认占位）。
...
### 阶段 2：scaffold（机械生成骨架）

```bash
python3 ~/.hermes/skills/text2md/scripts/plan.py scaffold <workspace>/source.txt
# --out-dir 缺省 = source.txt 所在目录（即工作区），无需指定
```

输出：
- `<workspace>/draft.md`：含 frontmatter + `![[IMG:header]]` 占位符 + 所有原文段落 + 完整版尾骨架
- `<workspace>/images.json`：必有图项的清单（header / section_titles / 版尾 6 件套）

### 阶段 3：agent 决定要插哪些 inline 配图 + 标记关键词
...
各自把图准备好（agent 用生图能力 / 用户从素材库找），按 `id` 命名存到 `<workspace>/images/`，例如：
- `<workspace>/images/header.png`
...
- `file`: 改为图片绝对路径
- `status`: 改为 `ready`
- 对 `grid_card`（推文卡）和 `header_banner`（版头）还要填 `link_url`
...
`plan.py` 会检查工作区合法性 + 所有占位符对应的 file 是否就绪 + 链接是否合法（拒绝 placeholder URL），缺一项就报错列出。全 ready 后输出 `<workspace>/final.md`。

```bash
python3 ~/.hermes/skills/md2wechat/scripts/render.py <workspace>/final.md
# → <workspace>/final.html，浏览器自动打开（含一键复制按钮）
```
...
```bash
# 阶段 0：创建工作区（推荐起步，自动检查/创建根目录）
python3 scripts/plan.py new "<文章主题>"

# 阶段 2：从文字稿生成骨架（--out-dir 缺省 = source.txt 所在目录）
python3 scripts/plan.py scaffold <workspace>/source.txt

# 阶段 5：占位符 → 真路径 → final.md
python3 scripts/plan.py fill <workspace>
```

环境变量：
- `WXMP_ARTICLES_DIR`：文章工作区根目录，默认 `~/wxmp-articles/`
